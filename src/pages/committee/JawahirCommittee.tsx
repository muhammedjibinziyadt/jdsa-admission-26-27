...
export default function JawahirCommittee() {
  ...
  return (
    <CommitteePageShell id="jawahir">
      {/* Magazines */}
      <section className="bg-white rounded-2xl shadow-sm border border-amber-100 p-5">
        <h3 className="text-sm font-semibold text-amber-800 flex items-center gap-2 mb-4">
          <BookOpen className="w-4 h-4" /> അൽ ജവാഹിർ മാഗസിൻ
        </h3>
        ...
        {isLoggedIn && (
          ...
          >
            ...
            <div>
              ...
              <Input type="date" value={mag.issue_date} onChange={(e) => setMag({ ...mag, issue_date: e.target.value })} className="rounded-lg" />
            </div>
            <div>
              <Label className="text-xs">Magazine PDF *</Label>
              <Input type="file" accept="application/pdf" onChange={(e) => setMagFile(e.target.files?.[0] || null)} className="rounded-lg" />
            </div>
            <div>
              <Label className="text-xs">Cover image (optional)</Label>
              <Input type="file" accept="image/*" onChange={(e) => setCoverFile(e.target.files?.[0] || null)} className="rounded-lg" />
            </div>
            <Button size="sm" type="submit" disabled={uploading} className="bg-amber-600 hover:bg-amber-700">
              {uploading ? <Loader2 className="w-3.5 h-3.5 mr-1 animate-spin" /> : <Upload className="w-3.5 h-3.5 mr-1" />} Upload Magazine
            </Button>
          </form>
        )}
        ...
        {magazines.loading ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : magazines.rows.length === 0 ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {magazines.rows.map((m) => (
              <div key={m.id} className="rounded-xl border border-gray-100 overflow-hidden bg-gray-50">
                {m.cover_url ? (
                  <img src={m.cover_url} alt={m.title} className="w-full aspect-[3/4] object-cover" />
                ) : (
                  <div className="w-full aspect-[3/4] bg-amber-100 flex items-center justify-center text-amber-400"><BookOpen className="w-10 h-10" /></div>
                )}
                <div className="p-2">
