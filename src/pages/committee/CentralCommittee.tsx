...
export default function CentralCommittee() {
  ...
  return (
    <CommitteePageShell id="central">
      {/* Inherit existing committee section content */}
      <CommitteeSection />
        ...
        {isLoggedIn && (
          <form
            onSubmit={async (e) => {
              ...
              const ok = await updates.insert({ title: updateForm.title, content: updateForm.content || null });
              if (ok) setUpdateForm({ title: '', content: '' });
            }}
            ...
            {updates.rows.map((u) => (
              <div key={u.id} className="p-3 bg-gray-50 rounded-lg border border-gray-100 flex justify-between gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  {u.content && <p className="text-xs text-gray-600 mt-0.5 whitespace-pre-wrap">{u.content}</p>}
                  <p className="text-xs text-gray-400 mt-1">{new Date(u.entry_date).toLocaleDateString('en-IN')}</p>
                </div>
                {isLoggedIn && (
      ...
      <section className="bg-white rounded-2xl shadow-sm border border-emerald-100 p-5">
        ...
        {isLoggedIn && (
          <form
            onSubmit={async (e) => {
              e.preventDefault();
              if (!usageForm.item_name.trim()) return;
              const ok = await usage.insert({
                item_name: usageForm.item_name,
                used_by: usageForm.used_by || null,
                quantity: usageForm.quantity ? Number(usageForm.quantity) : null,
                notes: usageForm.notes || null,
              });
              if (ok) setUsageForm({ item_name: '', used_by: '', quantity: '', notes: '' });
            }}
            className="space-y-2 mb-4 p-3 bg-emerald-50 rounded-xl"
          >
        ...
        {usage.loading ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : usage.rows.length === 0 ? (
          ...
          <div className="space-y-2">
            {usage.rows.map((u) => (
              <div key={u.id} className="p-3 bg-gray-50 rounded-lg border border-gray-100 flex justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <p className="font-medium text-gray-800 text-sm">{u.item_name}{u.quantity != null && <span className="text-gray-500"> × {u.quantity}</span>}</p>
                  {u.used_by && <p className="text-xs text-gray-600">By: {u.used_by}</p>}
                  {u.notes && <p className="text-xs text-gray-500">{u.notes}</p>}
                  <p className="text-xs text-gray-400 mt-1">{new Date(u.entry_date).toLocaleDateString('en-IN')}</p>
                </div>
      ...
      <section className="bg-white rounded-2xl shadow-sm border border-emerald-100 p-5">
        ...
        {isLoggedIn && (
          <form
            onSubmit={async (e) => {
              e.preventDefault();
              if (!minuteForm.notes.trim()) return;
              const ok = await minutes.insert({ meeting_date: minuteForm.meeting_date, notes: minuteForm.notes });
              if (ok) setMinuteForm({ meeting_date: new Date().toISOString().slice(0, 10), notes: '' });
            }}
            className="space-y-2 mb-4 p-3 bg-emerald-50 rounded-xl"
          >
            <div>
              <Label className="text-xs text-emerald-700">Meeting Date</Label>
              <Input type="date" value={minuteForm.meeting_date} onChange={(e) => setMinuteForm({ ...minuteForm, meeting_date: e.target.value })} className="rounded-lg" />
            </div>
          ...
          <div className="space-y-2">
            {minutes.rows.map((m) => (
              <div key={m.id} className="p-3 bg-gray-50 rounded-lg border border-gray-100 flex justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <p className="text-xs font-semibold text-emerald-700 mb-1">{new Date(m.meeting_date).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</p>
